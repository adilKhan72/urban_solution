...
namespace App\Http\Controllers\Principal\Profile;
...
class InformationController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('Principal_dashboard.profile.informations.index',['user'=>$user]);
    }
    ...
    { 
        ...
        if($request->form == "form_5"){
            ...
            try { 
                DB::table('users')
                ->where('id', Auth::user()->id)
                ->update(['joining_date' => $request->joining_date,'id_card_number' => $request->id_card_number,'description' => $request->description,]);
                $arr = array('msg' => 'Fields Updated Successfully', 'status' => true);
                $request->session()->flash('form_success', 'Fields Updated Successfully');
            } catch(\Illuminate\Database\QueryException $ex){ 
                $arr = array('msg' => $ex->getMessage(), 'status' => false);
